...
const RecommendedMoviesContainer = document.querySelector(
  ".Recommended-movies"
);
...
const MoviesAPI =
  "https://api.themoviedb.org/3/discover/movie?sort_by=popularity.des&api_key=fc95b085c87b910a96dbb74ba609c600&page=";

const searchUrl =
  "https://api.themoviedb.org/3/search/movie?api_key=fc95b085c87b910a96dbb74ba609c600&query=";
...
const options = {
  ...
  headers: {
    ...
    Authorization:
      "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJmYzk1YjA4NWM4N2I5MTBhOTZkYmI3NGJhNjA5YzYwMCIsInN1YiI6IjY1ZTZjMGRhOGQxYjhlMDE2MzY2ZDBlZSIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.nubfsmdkVnpIrodIFatI9C6dUsqO4F5xvZxvs4FHexY",
  },
};
  ...
  fetchFavoriteMovies().then((favoriteMovies) => {
    ...
    favoriteMovies.forEach((movie) => {
      ...
      MoviesDisplay.innerHTML = `
            ...
            `;
      MoviesDisplay.addEventListener("click", () => {
...
function showMovies(movies) {
  ...
  movies.slice(0, 12).forEach((movie) => {
    ...
    favoriteButton.addEventListener("click", (event) => {
      ...
      toggleFavorite(movieId);
      event.target.textContent = isFavorite(movieId)
        ? "Remove from Favorites"
        : "Add to Favorites";
    });
    // showMovieDetails(movie)
...
function showMovieDetails(movie) {
  ...
  document.getElementById(
    "detailPoster"
  ).innerHTML = `<img class="MovieImg"src="https://image.tmdb.org/t/p/w500/${movie.poster_path}" alt="${movie.title}">`; // Line 3
  document.getElementById(
    "detailRating"
...
// Function to generate event listeners dynamically for each genre
function generateGenreEventListeners() {
    genres.forEach(genre => {
        const button = document.getElementById(genre.name);
        if (button) {
            button.addEventListener('click', async () => {
                try {
                    const genreMovies = await fetchMoviesByGenre(genre.id);
                    showMovies(genreMovies)
                    // Call a function to display movies for the clicked genre
                    // For example: showMovies(genreMovies);
                } catch (error) {
...
// Call the function to generate event listeners for each genre
generateGenreEventListeners();

function GenerateRadioButton(){
    const container = document.querySelector('.containers')
    genres.slice(0,16).forEach(genre =>{
        const radioWrapper = document.createElement('div');
        radioWrapper.classList.add('radio-wrapper')
        radioWrapper.id = genre.name;
        ...
        divBtn.appendChild(genreText)


        const span2 = document.createElement('span');
        span2.classList.add('btn__glitch');
        span2.innerText = genre.name;
        divBtn.appendChild(span2);

        const label = document.createElement('label');
        label.classList.add('number');
        label.innerText = `r${genres.indexOf(genre) + 1}`;

        radioWrapper.appendChild(input);
        radioWrapper.appendChild(divBtn);
